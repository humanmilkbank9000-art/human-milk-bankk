...
@section('content')
    <div class="container-fluid page-container-standard">
        ...
        <div class="tab-content" id="requestTabContent" style="margin-top: 0;">
                                                ...
                                                <td data-label="Donor/Batch">
                                                    @if($request->dispensedMilk)
                                                        @php
                                                            ...
                                                            // Always include unpasteurized donation sources (with volumes)
                                                            if (isset($dm->sourceDonations) && $dm->sourceDonations->count() > 0) {
                                                                foreach ($dm->sourceDonations as $donation) {
                                                                    $label = null;
                                                                    if (($donation->relationLoaded('user') || $donation->user) && ($donation->user?->first_name || $donation->user?->last_name)) {
                                                                        $first = $donation->user->first_name ?? '';
                                                                        $last = $donation->user->last_name ?? '';
                                                                    ...
                                                                    if ($label) {
                                                                        $parts[] = $label . ' (' . ($donation->pivot->volume_used ?? 0) . 'ml)';
                                                                    } else {
                                                                        $parts[] = 'Donation #' . ($donation->breastmilk_donation_id ?? '-') . ' (' . ($donation->pivot->volume_used ?? 0) . 'ml)';
                                                                    }
                                                                }
                                                            }

                                                            // Pasteurized batches: only show pivot volumes when
                                                            // - more than one batch was used, OR
                                                            // - a specific batch was recorded on the dispensed row (`source_batch_id`)
                                                            if (isset($dm->sourceBatches) && $dm->sourceBatches->count() > 0) {
                                                                $showBatchVolumes = ($dm->sourceBatches->count() > 1) || !empty($dm->source_batch_id);
                                                                foreach ($dm->sourceBatches as $batch) {
                                                                    if ($showBatchVolumes) {
                                                                        $parts[] = 'Batch ' . ($batch->batch_number ?? '-') . ' (' . ($batch->pivot->volume_used ?? 0) . 'ml)';
                                                                    } else {
                                                                        $parts[] = 'Batch ' . ($batch->batch_number ?? '-');
                                                                    }
                                                                }
                                                            }
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <form id="dispensingForm{{ $request->breastmilk_request_id }}">
                                ...
                                <input type="hidden" id="milkTypeSelect{{ $request->breastmilk_request_id }}" name="milk_type"
                                    value="pasteurized">
                                                                    ...
                                                                    Swal.fire({
                                                                        ...
                                                                        html: `
                                                                            <div class="text-start">
                                                                                ...
                                                                                <label for="decline-reason" class="form-label"><strong>Reason for Declining <span class="text-danger">*</span></strong></label>
                                                                                <textarea id="decline-reason" class="form-control" rows="3" placeholder="Please provide a reason for declining this request..." required></textarea>
                                                                            </div>
                                                                        `,
                                                                        icon: 'warning',
                                                                        showCancelButton: true,
                                                                        ...
                                                                        if (result.isConfirmed) {
                                                                            ...
                                                                            // Submit the decline request with reason
                                                                            fetch(`{{ url('/admin/breastmilk-request') }}/${requestId}/decline`, {
                                                                                ...
                                                                                body: JSON.stringify({ admin_notes: declineReason })
                                                                            })
                                                                                .then(response => response.json())
                                                                                .then(data => {
                                                                                    if (data.error) {
                                                                    ...
                                                                    Swal.fire({
                                                                        ...
                                                                        html: `
                                                                            <div class="text-start">
                                                                                ...
                                                                                <p class="text-muted mb-3">This action cannot be undone.</p>
                                                                                <label for="reject-reason" class="form-label"><strong>Reason for Rejection <span class="text-danger">*</span></strong></label>
                                                                                <textarea id="reject-reason" class="form-control" rows="3" placeholder="Please provide a reason for rejecting this request..." required></textarea>
                                                                            </div>
                                                                        `,
                                                                        icon: 'warning',
                                                                        showCancelButton: true,
