...
@section('content')
    ...
    <div class="card shadow-sm rounded-lg border-0 mb-4" style="background: #fff;">
        <div class="card-header bg-primary text-white rounded-top d-flex flex-column align-items-start">
            <h5 class="mb-0">Completed Donations</h5>
            <small class="text-white-50">All your successfully completed donations</small>
        </div>
        <div class="card-body py-4">
            @if($completedDonations->count() > 0)
                <div class="table-responsive">
                    <table class="table table-standard table-hover align-middle">
                                ...
                                <tr class="bg-white shadow-sm rounded">
                                    <td class="text-center">
                                        @if($donation->donation_method === 'walk_in')
                                            <span class="badge-status badge-donation-walkin">Walk-in</span>
                                        @else
                                            <span class="badge-status badge-donation-home">Home Collection</span>
                                        @endif
                                    </td>
                                    <td class="text-center">
                                    ...
                                    <td class="text-center">
                                        @if($donation->donation_method === 'walk_in')
                                            {{ $donation->availability ? $donation->availability->formatted_time : ($donation->donation_time ?? 'N/A') }}
                        ...
                        <div class="flex-fill" style="min-width: 250px; max-width: 350px;">
                            <div class="card shadow-sm rounded-lg border-0 bg-light h-100">
                                <div class="card-body text-center py-4">
                                    ...
                                    <h2 class="text-success fw-bold mb-0">{{ $completedDonations->sum('total_volume') }} ml</h2>
                                </div>
                            </div>
                        </div>
                        <div class="flex-fill" style="min-width: 250px; max-width: 350px;">
